        ...
        if (toolId === 'organize') {
            ...
            deleteBtn.addEventListener('click', (e) => {
                if (container.children.length <= 1) {
                    showAlert('Keep one page', 'A PDF needs at least one page.');
                    return;
                }
...
`,
    ...
    <p class="mb-6 text-gray-400">Turn one PDF into smaller files by range, page, or visual selection.</p>
    ${createFileInputHTML()}
...
`,
    ...
    organize: () => `
        ...
        <button id="process-btn" class="btn-gradient w-full mt-6" disabled>Save Changes</button>
    `,
    ...
    rotate: () => `
        ...
        <button id="process-btn" class="btn-gradient w-full mt-6" disabled>Save Rotations</button>
    `,
...
compress: () => `
    ...
    <p class="mb-6 text-gray-400">Make scanned and image-heavy PDFs smaller. Text will become part of the page image.</p>
    ${createFileInputHTML()}
    <div id="file-display-area" class="mt-4 space-y-2"></div>
    <div id="compress-options" class="hidden tool-options mt-6">
        <label for="compression-level" class="block mb-2 text-sm font-medium text-gray-300">Choose the balance</label>
        <select id="compression-level" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg p-2.5">
            <option value="balanced">Balanced — recommended</option>
            <option value="smaller">Smaller file — softer pages</option>
            <option value="quality">Better quality — larger file</option>
        </select>
        <p class="tool-note mt-3">Best for scans, worksheets, and PDFs made from photos. A compact text-only PDF may not get smaller.</p>
        <button id="process-btn" class="btn-gradient w-full mt-4" disabled>Compress and download</button>
    </div>
`,
    'pdf-to-greyscale': () => `
...
`,
    ...
    'image-to-pdf': () => `
        <h2 class="text-2xl font-bold text-white mb-4">Images to PDF</h2>
        <p class="mb-6 text-gray-400">Put JPG, PNG, or WebP images into one PDF. Drag the previews to change their order.</p>
        ...
        <button id="process-btn" class="btn-gradient w-full mt-6" disabled>Create PDF</button>
    `,

    'pdf-to-images': () => `
        <h2 class="text-2xl font-bold text-white mb-4">PDF to Images</h2>
        <p class="mb-6 text-gray-400">Turn every PDF page into an image and download them together in a ZIP file.</p>
        ${createFileInputHTML()}
        <div id="file-display-area" class="mt-4 space-y-2"></div>
        <div id="images-options" class="hidden tool-options mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label for="image-format" class="block mb-2 text-sm font-medium text-gray-300">Image format</label>
                <select id="image-format" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg p-2.5">
                    <option value="jpg">JPG — smaller files</option>
                    <option value="png">PNG — sharper text</option>
                </select>
            </div>
            <div>
                <label for="image-quality" class="block mb-2 text-sm font-medium text-gray-300">Image size</label>
            ...
            <p class="tool-note sm:col-span-2">Large works well for sharing and printing. Extra large uses more memory on long PDFs.</p>
            <button id="process-btn" class="btn-gradient w-full sm:col-span-2" disabled>Download page images</button>
        </div>
    `,
