...
let imageListSortable: Sortable | null = null;
...
function handleMultiFileUpload(toolId) {
    ...
    if (toolId === 'merge') {
        ...
        state.files.forEach((file, index) => {
            ...
            const li = document.createElement('li');
            li.className = "image-sheet relative group cursor-move";
            li.dataset.fileIndex = index.toString();
            ...
            p.className = "image-sheet__name absolute bottom-0 left-0 right-0 text-xs text-center truncate p-1";
        ...
        imageListSortable?.destroy();
        imageListSortable = Sortable.create(imageList);
    }
}
