...
import { getImagePdfPageSize } from '../utils/conversionOptions.js';
...
export async function imageToPdf() {
    ...
    try {
        ...
        const sortedFiles = Array.from(imageList.children)
            ...
            .map(li => state.files[Number(li.dataset.fileIndex)])
            .filter(Boolean);
        ...
        for (const file of sortedFiles) {
            ...
            if (file.type === 'image/jpeg') {
                // Browser decoding normalizes phone-photo EXIF orientation.
            ...
            const pageSize = getImagePdfPageSize(image.width, image.height);
